...
# 2. 초기화
npx cap init "ggg" "app.ggg.travel" --web-dir dist
...
| API | **Supabase Edge Functions (Deno)** | 예보 갱신, ggg score 계산 |
...
│   ├── weather/            # 날씨 관련 컴포넌트
│   └── layout/             # 사이드바, 토박, 하단 탭
├── hooks/                  # 커스텀 훅 (useWeather, useGggScore 등)
├── lib/
...
const NAV_ITEMS = [
  ...
  { id: 'score',    icon: '🌡️', label: '스코어',  screen: 'GggScore' },
...
<GggScoreBadge />  - 76/100 점수 + 퍼센타일 바
<StatCard />           - 기온/PM2.5/강수/쾌적도 지표 카드
<ForecastStrip />      - 5~14일 예보 가로 스크롤
<MonthlyBarChart />    - ggg score 월별 막대 차트 (Recharts)
<PercentileChart />    - 역사 기후 분포 바 (최악/일반/최상)
<ScoreCalendar />      - 날짜별 ggg score/추천도 캘린더 (색상 히트맵 + 등급 배지)
...
| 적용 화면 | ggg score TCI 보강, 홈 날씨 지수 |
...
**TCI 활용 전략**
```ts
// ggg score = best_travel_week.travel_score (주 기반)
// TCI = 기상청 관광기후지수 (일 기반, 한국 도시 한정)
// → 국내 도시에 한해 TCI로 ggg score 보정 레이어 추가
...
| ggg (코어) | `climate_normals` | ✅ 완료 | 365행/도시 |
| ggg (코어) | `monthly_climate` | ✅ 완료 | 12행/도시 |
...
**네이버** Local Search / Places 계열에서 가져온 주변 POI를 위치·도시 단위로 캐시한다. (`place_id`는 네이버 식별자로 매핑)

```sql
CREATE TABLE nearby_places (
  id              BIGSERIAL PRIMARY KEY,
  city_id         TEXT REFERENCES cities(id),
  place_id        TEXT NOT NULL,           -- 외부 POI id (예: 네이버 mapId 등 — 구현 시 확정)
...
#### [FEATURE] `climate_score_monthly` — ggg score 월별 캐시
...
### 4-3. ggg score 계산 로직 정의

```
ggg score = (TravelScore × 100) 반올림
...
| **ggg score** | `best_travel_week`, `climate_normals`, `monthly_climate`, `climate_score_monthly` | — | 연인 레이어 ON → D-day 저장 CTA 추가. 가족 레이어 ON → 안전 지수 강조 |
...
| 레이어 | 추가 데이터 | 적용 화면 |
|---|---|---|
| 연인 ON | `forecast_weather.sunrise`, `forecast_weather.sunset` → 골든아워 계산 | 홈, ggg score, D-day |
...
```
비로그인 (anon):    도시 검색, 날씨 조회, ggg score 확인
...
- [ ] ggg score 화면 (best_travel_week + climate_normals)
- [ ] ggg score: 날짜 선택 캘린더에 일자별 Score + 추천도 배지 시각화
