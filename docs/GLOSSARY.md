...
제품·저장소·문서 스위트의 기준 명칭. 과거 잠정 코드네임 **CLIMATE**는 이관 중이다.
...
- 관련 컴포넌트: `<GggScoreBadge />`, `<ScoreCalendar />`
...
### best_travel_week
도시별 주간 단위 ggg score 최상위 구간 테이블. 장소 추천/스코어의 1차 소스.

### 숨은 황금 시즌 · `hidden_season_highlights`
과거 10년 데이터에서 "ggg score 80+ 이면서 관광객 수 평균 이하"인 구간을 별도 추출한 큐레이션.
- 화면: `/hidden-season`
...
### climate_normals
도시별 30년 평균 기후 (월/주/일 단위). ggg score 계산의 기준값.
...
- 앱·랜딩 등 제품명: **ggg** (필요 시 부제로 여행/기후 맥락 보조)
- 종합 점수 UI 표기: **ggg score**(소문자 `ggg` + 공백 + `score` 권장). 상표 기호(™)는 사용하지 않는다.
